...
func decodePrecoverageClosureInput(path string, resolve ...func(string) (money.Currency, error)) (ledger.CloseStatementAccountBeforeCoverageInput, error) {
	if path == "" || path == "-" {
...
func newStatementAccountLifecycleCommand(opts *options) *cobra.Command {
	...
	closeBeforeCoverage := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			input, err := decodePrecoverageClosureInput(inputPath, func(key string) (money.Currency, error) { return targetCurrency(cmd, store, "statement", key) })
			if err != nil {
				return err
			}
