...
func newSpendCommand(opts *options) *cobra.Command {
	...
	command := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			amount, err := positiveMoney(args[0], context.currency)
			if err != nil {
				return err
			}
...
func prepareHumanJournal(cmd *cobra.Command, opts *options, file journalFile) (*postingContext, ledger.CreateJournalInput, error) {
	...
	input, err := file.ledgerInput(context.currency)
	if err != nil {
		_ = context.store.Close()
		return nil, ledger.CreateJournalInput{}, err
	}
	...
	if debits == 0 || debits != credits {
		_ = context.store.Close()
		return nil, ledger.CreateJournalInput{}, apperr.New(apperr.Validation, "JOURNAL_UNBALANCED", fmt.Sprintf("debits %s must equal credits %s", context.currency.Format(debits), context.currency.Format(credits)))
	}
...
func loadPostingContext(cmd *cobra.Command, opts *options, dateText string) (*postingContext, error) {
	...
	return &postingContext{currency: outputCurrency(cmd), store: store, resolved: alias, accounts: accounts, period: *selected, date: date}, nil
}
...
func commitHumanJournal(cmd *cobra.Command, opts *options, context *postingContext, input ledger.CreateJournalInput, draft bool) error {
	...
	preview := humanTransactionFromInput(context.resolved.Key, input, draft, opts.dryRun, context.currency)
	if opts.dryRun {
...
func validateHumanJournal(context *postingContext, input ledger.CreateJournalInput) error {
	...
	if debits == 0 || debits != credits {
		return apperr.New(apperr.Validation, "JOURNAL_UNBALANCED", fmt.Sprintf("debits %s must equal credits %s", context.currency.Format(debits), context.currency.Format(credits)))
	}
	return nil
}

func humanTransactionFromInput(company string, input ledger.CreateJournalInput, draft, dryRun bool, currency money.Currency) humanTransaction {
	status := "POSTED"
	if draft {
		status = "DRAFT"
	}
	...
	result := humanTransaction{Currency: currency, Company: company, Book: input.Book, Kind: input.Kind, Date: input.PostingDate, Period: input.Period, Status: status, Description: input.Description, Reference: input.Reference, DryRun: dryRun}
	if result.Kind == "" {
...
func humanTransactionFromJournal(company string, journal ledger.Journal) humanTransaction {
	result := humanTransaction{Currency: journal.Currency, Company: company, Book: journal.BookCode, Number: journal.EntryNumber, Kind: journal.Kind, Date: journal.PostingDate, Period: journal.PeriodCode, Status: journal.Status, Description: journal.Description, Reference: journal.Reference, TotalDebitCents: journal.TotalDebitCents, TotalCreditCents: journal.TotalCreditCents}
	...
	list := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			for _, journal := range journals {
				...
				rows = append(rows, []string{strconv.FormatInt(journal.EntryNumber, 10), journal.PostingDate, journal.Status, journal.Description, journal.Reference, journal.Currency.Format(journal.TotalDebitCents)})
			}
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if opts.dryRun {
				result := humanCorrection{
					...
					Reversal:    humanTransactionFromInput(resolved.Key, reversalInput, draft, true, context.currency),
					Replacement: humanTransactionFromInput(resolved.Key, replacementInput, draft, true, context.currency),
				}
...
func reverseHumanTransaction(cmd *cobra.Command, opts *options, number, dateText, memo string, draft, undo bool) error {
	...
	if opts.dryRun {
		...
		return writeHumanTransaction(cmd, opts, humanTransactionFromInput(resolved.Key, input, draft, true, original.Currency))
	}
	store, err = openWrite(cmd, opts)
	if err != nil {
		return err
