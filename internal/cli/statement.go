...
func newStatementAccountCommand(opts *options) *cobra.Command {
	...
	create.Flags().StringVar(&currency, "currency", "", "account currency (defaults to the book currency)")
...
func parseStatementImport(data []byte, resolvers ...func(string) (money.Currency, error)) (ledger.StatementImportInput, error) {
	...
	if len(resolvers) > 0 {
		var err error
		currency, err = resolvers[0](file.StatementAccount)
		if err != nil {
			return ledger.StatementImportInput{}, err
		}
	}
	...
	for i, value := range file.Transactions {
		amount, err := currency.Parse(value.Amount)
		if err != nil {
...
func newStatementCommand(opts *options) *cobra.Command {
	...
	importCommand := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			defer func(closer interface{ Close() error }) { _ = closer.Close() }(store)
			input, err := parseStatementImport(data, func(key string) (money.Currency, error) { return targetCurrency(cmd, store, "statement", key) })
			if err != nil {
				return err
			}
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			for _, record := range result {
				...
				if record.AmountCents != nil {
					amount = record.Currency.Format(*record.AmountCents)
				}
	...
	show := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if result.AmountCents != nil {
				amount = result.Currency.Format(*result.AmountCents)
			}
			return writeResult(cmd, opts.format, result,
