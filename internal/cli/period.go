...
func newPeriodCommand(opts *options) *cobra.Command {
	...
	yearClose := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			defer func(closer interface{ Close() error }) { _ = closer.Close() }(store)
			if _, err := targetCurrency(cmd, store, "book", yearCloseBook); err != nil {
				return err
			}
			...
			return writeResult(cmd, opts.format, result,
				...
				[][]string{{strings.ToUpper(yearCloseBook), fmt.Sprint(closeYear), outputCurrency(cmd).Format(result.NetIncome), journalID, fmt.Sprint(result.DryRun)}})
		},
	}
