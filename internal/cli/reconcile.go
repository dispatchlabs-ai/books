...
func reconciliationRows(value ledger.Reconciliation) [][]string {
	return [][]string{{value.ID, value.StatementAccountCode, value.StartDate, value.EndDate,
		value.Currency.Format(value.BeginningBalanceCents), value.Currency.Format(value.LedgerBeginningCents), value.Currency.Format(value.OpeningOutstandingCents), value.Currency.Format(value.BeginningDifferenceCents),
		value.Currency.Format(value.StatementActivityCents), value.Currency.Format(value.EndingBalanceCents), value.Currency.Format(value.CalculatedEndingCents),
		value.Currency.Format(value.StatementDifferenceCents), value.Currency.Format(value.LedgerEndingCents), value.Currency.Format(value.EndingOutstandingCents), value.Currency.Format(value.LedgerDifferenceCents),
...
func newReconcileCommand(opts *options) *cobra.Command {
	...
	start := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			defer func(closer interface{ Close() error }) { _ = closer.Close() }(store)
			currency, err := targetCurrency(cmd, store, "statement", account)
			if err != nil {
				return err
			}
			beginning, err := currency.Parse(beginningText)
			if err != nil {
				return apperr.Wrap(apperr.Invalid, "BALANCE_INVALID", "invalid beginning balance", err)
			}
			ending, err := currency.Parse(endingText)
			if err != nil {
	...
	allocate := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			defer func(closer interface{ Close() error }) { _ = closer.Close() }(store)
			currency, err := targetCurrency(cmd, store, "reconciliation", reconciliationID)
			if err != nil {
				return err
			}
			allocation, err := currency.Parse(allocationText)
			if err != nil {
			...
			data := map[string]any{"id": id, "reconciliation_id": reconciliationID, "statement_transaction_id": transactionID, "journal_line_id": lineID, "allocated_amount_cents": allocation, "currency": currency}
			return writeResult(cmd, opts.format, data, []string{"ALLOCATION", "RECONCILIATION", "TRANSACTION", "JOURNAL LINE", "AMOUNT"}, [][]string{{id, reconciliationID, transactionID, lineID, currency.Format(allocation)}})
		},
	}
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			for _, allocation := range result {
				rows = append(rows, []string{allocation.ID, allocation.StatementPostedDate, allocation.StatementExternalID,
					...
					allocation.JournalLineID, allocation.Currency.Format(allocation.AllocatedAmountCents)})
			}
