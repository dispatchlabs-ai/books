...
func journalRows(journal ledger.Journal) [][]string {
	...
	for _, line := range journal.Lines {
		rows = append(rows, []string{journal.BookCode, fmt.Sprint(journal.EntryNumber), journal.PostingDate, journal.Status, fmt.Sprint(line.LineNumber), line.AccountCode, line.Description, journal.Currency.Format(line.DebitCents), journal.Currency.Format(line.CreditCents), line.ID})
	}
	if len(rows) == 0 {
		rows = append(rows, []string{journal.BookCode, fmt.Sprint(journal.EntryNumber), journal.PostingDate, journal.Status, "", "", journal.Description, journal.Currency.Format(0), journal.Currency.Format(0), journal.ID})
	}
	return rows
}
...
func newJournalCommand(opts *options) *cobra.Command {
	...
	create := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			currency, err := targetCurrency(cmd, store, "book", file.Book)
			if err != nil {
				return err
			}
	...
	validate := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err := writeResult(cmd, opts.format, result, []string{"ID", "VALID", "DEBITS", "CREDITS", "ERRORS"}, [][]string{{result.JournalID, fmt.Sprint(result.Valid), result.Currency.Format(result.DebitCents), result.Currency.Format(result.CreditCents), errorsText}}); err != nil {
				return err
			}
			if !result.Valid {
				if errorsText == "" {
	...
	post := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			if opts.dryRun {
				...
				return writeResult(cmd, opts.format, map[string]any{"validation": result, "dry_run": true}, []string{"ID", "VALID", "DEBITS", "CREDITS", "DRY RUN"}, [][]string{{journalID, "true", result.Currency.Format(result.DebitCents), result.Currency.Format(result.CreditCents), "true"}})
			}
			store, err := openWrite(cmd, opts)
			if err != nil {
				return err
	...
	list := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			for _, value := range journals {
				rows = append(rows, []string{value.BookCode, fmt.Sprint(value.EntryNumber), value.PostingDate, value.Status, value.Description, value.Currency.Format(value.TotalDebitCents), value.ID})
			}
	...
	importCommand := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			defer func(closer interface{ Close() error }) { _ = closer.Close() }(store)
			input, err := parseJournalImport(raw, func(key string) (money.Currency, error) { return targetCurrency(cmd, store, "book", key) })
			if err != nil {
				return err
			}
...
func parseJournalImport(raw []byte, resolvers ...func(string) (money.Currency, error)) (ledger.JournalImportInput, error) {
	...
	for i, record := range file.Records {
		...
		if len(resolvers) > 0 {
			var err error
			currency, err = resolvers[0](record.Journal.Book)
			if err != nil {
				return result, err
			}
		}
