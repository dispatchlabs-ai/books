...
func newRootCommand() (*cobra.Command, *options) {
	...
	root.AddCommand(newMCPCommand(), newCashForecastCommand(opts), newBudgetCommand(opts),
		newInitCommand(opts),
...
func bindResolvedCompanyDatabase(cmd *cobra.Command, opts *options, store *storesqlite.Store) error {
	...
	if err != nil {
		return apperr.New(apperr.Invalid, "CURRENCY_NOT_SUPPORTED", "unsupported company currency")
	}
