...
func runManualReconciliationPlan(cmd *cobra.Command, opts *options, accountSelector, targetID, through, startText, beginningText, endingText, cleared, outputPath string) error {
	...
	ending, err := parseStatementBalance(endingText, statementAccount.Kind, outputCurrency(cmd))
	if err != nil {
	...
	plan := manualReconciliationPlan{
		Schema: currencyPlanSchema(manualReconciliationPlanSchema, "books.reconciliation-plan/v4", resolved.Company.Currency), Currency: planCurrency(resolved.Company.Currency), Company: resolved.Key, Book: resolved.Company.BookCode,
	...
	if plan.BeginningBalanceCents+plan.ActivityCents != plan.EndingBalanceCents {
		plan.Blockers = append(plan.Blockers, fmt.Sprintf("cleared activity produces statement ending %s, not %s", outputCurrency(cmd).Format(plan.BeginningBalanceCents+plan.ActivityCents), outputCurrency(cmd).Format(plan.EndingBalanceCents)))
	}
	...
	if plan.LedgerBeginningCents != plan.AdjustedBeginningCents {
		plan.Blockers = append(plan.Blockers, fmt.Sprintf("book beginning %s does not equal statement beginning plus opening outstanding items %s", outputCurrency(cmd).Format(plan.LedgerBeginningCents), outputCurrency(cmd).Format(plan.AdjustedBeginningCents)))
	}
	if plan.LedgerEndingCents != plan.AdjustedEndingCents {
		plan.Blockers = append(plan.Blockers, fmt.Sprintf("book ending %s does not equal statement ending plus remaining outstanding items %s", outputCurrency(cmd).Format(plan.LedgerEndingCents), outputCurrency(cmd).Format(plan.AdjustedEndingCents)))
	}
...
func newManualReconciliationApplyCommand(opts *options) *cobra.Command {
	...
	command := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if (plan.Schema != manualReconciliationPlanSchema && plan.Schema != "books.reconciliation-plan/v4") || !plan.Ready {
			...
			if !validPlanCurrency(plan.Schema, manualReconciliationPlanSchema, "books.reconciliation-plan/v4", plan.Currency, resolved.Company.Currency) {
				return apperr.New(apperr.Invalid, "PLAN_CURRENCY_MISMATCH", "plan currency does not match the company's immutable currency")
			}
...
func manualReconciliationBoundary(cmd *cobra.Command, store *storesqlite.Store, account ledger.StatementAccount, startText, beginningText, excludedReconciliationID string) (string, int64, *manualReconciliationPrior, []string, error) {
	...
	if strings.TrimSpace(beginningText) != "" {
		beginning, err = parseStatementBalance(beginningText, account.Kind, outputCurrency(cmd))
		if err != nil {
	...
	if priorExists && beginning != priorEnding {
		blockers = append(blockers, fmt.Sprintf("beginning balance must carry forward %s from the prior reconciliation", outputCurrency(cmd).Format(priorEnding)))
	}
