...
func newYearClosePlanCommand(opts *options) *cobra.Command {
	...
	command := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			plan := yearClosePlan{
				Schema: currencyPlanSchema(yearClosePlanSchema, "books.year-close-plan/v2", resolved.Company.Currency), Currency: planCurrency(resolved.Company.Currency), Company: resolved.Key, Book: resolved.Company.BookCode,
...
func newYearCloseApplyCommand(opts *options) *cobra.Command {
	...
	command := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if !validPlanCurrency(plan.Schema, yearClosePlanSchema, "books.year-close-plan/v2", plan.Currency, resolved.Company.Currency) {
