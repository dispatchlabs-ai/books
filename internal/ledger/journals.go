...
func (s *Service) createJournal(ctx context.Context, raw CreateJournalInput, post bool) (Journal, error) {
	...
	result, err := s.createJournalTx(ctx, tx, raw, post)
	if err != nil {
		return Journal{}, err
	}
...
// createJournalTx is shared by standalone journal operations and atomic bank imports.
func (s *Service) createJournalTx(ctx context.Context, tx *sql.Tx, raw CreateJournalInput, post bool) (Journal, error) {
	...
	if input.SourceSystem != "" {
		...
		if err == nil {
			...
			return getJournal(ctx, tx, existingID)
		}
...
func getJournal(ctx context.Context, q queryer, journalID string) (Journal, error) {
	var journal Journal
	err := q.QueryRowContext(ctx, `SELECT je.id, je.book_id, b.code, b.currency, je.entry_number, je.kind, je.posting_date,
		...
		&journal.ID, &journal.BookID, &journal.BookCode, &journal.Currency, &journal.EntryNumber, &journal.Kind, &journal.PostingDate,
	...
	rows, err := q.QueryContext(ctx, `SELECT jl.id, jl.line_number, jl.account_id, a.code, a.name,
...
func validateJournalQuery(ctx context.Context, q queryer, journalID string) (JournalValidation, error) {
	...
	err := q.QueryRowContext(ctx, `SELECT status, kind, book_id, posting_date, period_id,
		COALESCE(reversal_of_id, ''), COALESCE(tax_type, ''), COALESCE(tax_accounting_period, ''), (SELECT currency FROM books WHERE books.id=journal_entries.book_id)
		FROM journal_entries WHERE id = ?`, journalID).Scan(
		&status, &kind, &bookID, &postingDate, &periodID, &reversalID, &taxType, &taxAccountingPeriod, &result.Currency,
	)
	if err == sql.ErrNoRows {
...
func (s *Service) ListJournals(ctx context.Context, bookCode, from, to, status string) ([]Journal, error) {
	query := `SELECT je.id, je.book_id, b.code, b.currency, je.entry_number, je.kind, je.posting_date, fp.code, je.status,
	...
	for rows.Next() {
		var journal Journal
		if err := rows.Scan(&journal.ID, &journal.BookID, &journal.BookCode, &journal.Currency, &journal.EntryNumber,
