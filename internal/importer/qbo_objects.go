...
func populatePurchase(catalog *accountCatalog, object qboObject, journal *rawJournal) (bool, error) {
	...
	total, err := numberCents(object.TotalAmount, "purchase TotalAmt", object.CurrencyRef.Value)
	if err != nil {
		return false, err
	}
	var lineTotal int64
	for index, line := range object.Lines {
		...
		amount, err := numberCents(line.Amount, fmt.Sprintf("purchase line %d Amount", index+1), object.CurrencyRef.Value)
		if err != nil {
			return false, err
		}
		if amount < 0 {
...
func populateDeposit(catalog *accountCatalog, object qboObject, journal *rawJournal) (bool, error) {
	...
	total, err := numberCents(object.TotalAmount, "deposit TotalAmt", object.CurrencyRef.Value)
	if err != nil {
		return false, err
	}
	var lineTotal int64
	for index, line := range object.Lines {
		...
		amount, err := numberCents(line.Amount, fmt.Sprintf("deposit line %d Amount", index+1), object.CurrencyRef.Value)
		if err != nil {
			return false, err
		}
		lineTotal += amount
...
func populateInvoice(catalog *accountCatalog, object qboObject, journal *rawJournal) (bool, error) {
	...
	total, err := numberCents(object.TotalAmount, "invoice TotalAmt", object.CurrencyRef.Value)
	if err != nil {
		return false, err
	}
	var lineTotal int64
	for index, line := range object.Lines {
		...
		amount, err := numberCents(line.Amount, fmt.Sprintf("invoice line %d Amount", index+1), object.CurrencyRef.Value)
		if err != nil {
			return false, err
		}
		lineTotal += amount
...
func populatePayment(catalog *accountCatalog, object qboObject, journal *rawJournal) (bool, error) {
	...
	total, err := numberCents(object.TotalAmount, "payment TotalAmt", object.CurrencyRef.Value)
	if err != nil {
		return false, err
	}
	var allocationTotal int64
	for index, line := range object.Lines {
		amount, err := numberCents(line.Amount, fmt.Sprintf("payment line %d Amount", index+1), object.CurrencyRef.Value)
		if err != nil {
			return false, err
		}
...
func populateJournalEntry(catalog *accountCatalog, object qboObject, journal *rawJournal) (bool, error) {
	for index, line := range object.Lines {
		...
		amount, err := numberCents(line.Amount, fmt.Sprintf("journal-entry line %d Amount", index+1), object.CurrencyRef.Value)
		if err != nil {
			return false, err
		}
		if amount < 0 {
...
func populateTransfer(catalog *accountCatalog, object qboObject, journal *rawJournal) (bool, error) {
	...
	amount, err := numberCents(object.Amount, "transfer Amount", object.CurrencyRef.Value)
	if err != nil {
		return false, err
	}
	if amount == 0 {
...
func numberCents(number json.Number, field string, currencies ...string) (int64, error) {
	if strings.TrimSpace(number.String()) == "" {
		return 0, objectContentError{"QBO_AMOUNT_MISSING", field + " is missing"}
	}
	cents, err := parseCents(number.String(), currencies...)
	if err != nil {
