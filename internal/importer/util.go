...
func parseCents(value string, currencies ...string) (int64, error) {
	match := decimalPattern.FindStringSubmatch(strings.TrimSpace(value))
	...
	amount := match[1] + whole
	if match[3] != "" {
		amount += "." + match[3]
	}
	return money.ParseCurrency(amount, code)
}
...
func validateRequest(request Request) error {
	...
	for index := range request.Entities {
		...
		if !money.IsSupportedCurrency(entity.Currency) {
			return fmt.Errorf("entity %s uses unsupported currency %q", entity.EntityCode, entity.Currency)
		}
