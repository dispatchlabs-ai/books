...
func parseJournalXLSX(state *entityState, source Source) error {
	...
	finalize := func() {
		...
		for _, sourceLine := range current.lines {
			...
			debit, err := parseOptionalCents(sourceLine.debit, state.request.Currency)
			if err != nil {
			...
			credit, err := parseOptionalCents(sourceLine.credit, state.request.Currency)
			if err != nil {
				appendDiagnostic(&state.diagnostics, SeverityError, "XLSX_AMOUNT_INVALID", state.request, source.Path,
...
func parseOptionalCents(value string, currencies ...string) (int64, error) {
	if strings.TrimSpace(value) == "" {
		return 0, nil
	}
	return parseCents(value, currencies...)
}
