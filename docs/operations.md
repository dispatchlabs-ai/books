...
Stop the service before database replacement or migration, then restart it to
revalidate company bindings and reload grants. Queued uploads resume after a
restart. Keep the returned job and preview IDs; use `bank-import show JOB` to
inspect durable status, and retry the same apply digest to recover a lost
response. A stale preview needs a new key and fresh review. FAILED source files
are retained for diagnosis and cannot be deleted through the API.

Import evidence, including immutable source options, is part of schema-v2 backups.
Changing options for an already imported file requires resolving the prior source
interpretation; an alternate upload cannot silently reinterpret its batch.
Use `bank-import matches` and explicit mapping decisions to resolve overlap
between different statement exports before preview. Run Doctor and audit verification
