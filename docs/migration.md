...
Books currently uses schema version 2. The unchanged native schema in
`0001_initial.sql` is followed by `0002_bank_import_jobs.sql`, which adds durable
uploads and immutable previews. Fresh creation and forward migration from v1
record both checksum-bound migrations in `schema_migrations`. Back up with the
old binary before upgrading, stop writers, run `books db migrate --dry-run`,
then `books db migrate`, followed by `books doctor`. The HTTP service
refuses unmigrated databases. Downgrade is not supported; recovery uses the
pre-upgrade backup and matching binary.
...
6. Create a fresh company through Books, then reproduce the
