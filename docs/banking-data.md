...
The [OFX client workflow](api.md) accepts bank/card statement files through
`bank-import` or the HTTP API. It retains raw bytes, timestamps, balances, and
native fields, discovers accounts, requires explicit company-local mappings,
and offers optional explicit journal classifications. No balance is treated as
a reconciliation certificate or opening-balance journal. Overlapping FITIDs
reuse existing materializations; unsupported corrections fail closed.
