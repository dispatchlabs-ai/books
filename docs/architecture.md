...
## Headless client boundary

The CLI and HTTP transports share company-bound `internal/application`
workflows over `internal/ledger`. Only the authenticated HTTP principal sets the
API actor. Registry UUID/book/entity binding prevents a client from selecting a
filesystem path or arbitrary book. `internal/banking` parses bounded statement evidence
without choosing accounting classifications. Durable import jobs and immutable
previews live in schema v2. Preview runs ledger operations under a savepoint and
rolls them back; apply revalidates the ledger revision and commits the same path
in one transaction. See [client API](api.md) for scope and wire contracts.

Statement adapters normalize source currency, status, controls, and nested details
without additional postings. Immutable upload audit evidence stores source options;
ledger-owned identity review binds explicit duplicate/new decisions into previews.
Pending and review observations use the existing source lifecycle. See
[statement semantics](statement-formats.md#identity-and-posting).
