...
## Headless service and statement files

`books serve --server-config PRIVATE_JSON` runs the authenticated company-scoped
HTTP API and durable parser worker. Company grants are configured in that file;
`serve` does not accept `--company` or `--db` scope overrides.

`books bank-import formats|upload|show|process|matches|preview|plan|apply` shares the same
application services. See the [complete workflow and contracts](api.md).

Use `bank-import formats` to list supported profiles. `upload --options profile.json`
