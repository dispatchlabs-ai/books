...
For an agent-led introduction, start with [the README](../README.md) and
[agent workflows](agent-workflows.md). This guide collects the command examples
for operators and agents who need the details.
...
Choose one supported ISO monetary currency at initialization with `--currency`, for example `EUR`, `JPY`, or `KWD`; USD remains the default. The currency determines exact decimal precision and applies to every account, journal, import, reconciliation, and report in that entity. See [currency rules and compatibility](currencies.md).
...
Repository development and tests must set `BOOKS_HOME` to a new disposable directory. They must never probe `~/.books` or an existing SQLite file.
...
- Money is stored only as signed 64-bit integer minor units of the owning currency; accounting never uses floating point.
...
Read [CONTRIBUTING.md](../CONTRIBUTING.md) before proposing a change and [AGENTS.md](../AGENTS.md) before using a coding agent to modify this repository. For bookkeeping, follow [agent workflows](agent-workflows.md). Report vulnerabilities through the private process in [SECURITY.md](../SECURITY.md), not a public issue.

Books is licensed under the [MIT License](../LICENSE).

See [CLI workflows](cli.md), [architecture](architecture.md), [banking data standard](banking-data.md), [operations](operations.md), and [migration controls](migration.md) for more detail.
